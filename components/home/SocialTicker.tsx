    ...
    <div className="v3-ticker-bar" role="marquee" aria-label="Recent card activity">
      ...
      <div className="v3-ticker-wrap">
        <div className="v3-ticker-track">
          {ALL.map((event, index) => (
            <span key={index} className="v3-ticker-event">
              <span style={{ fontSize: "1rem" }}>{event.avatar}</span>
              <span className="name">{event.name}</span>
              <span style={{ color: "var(--v3-border)" }}>sent</span>
              <span className="cname">&quot;{event.card}&quot;</span>
              <span style={{ opacity: 0.5, fontSize: "0.7rem" }}>{event.time}</span>
              {index < ALL.length - 1 && (
                <span style={{ color: "var(--v3-border)", padding: "0 8px" }}>·</span>
              )}
            </span>
          ))}
        </div>
